use actix_web::{delete, error, get, post, put, web, HttpRequest};
...
use crate::lib::{get_url_querys, AppState};
use crate::models;
...
async fn get_todo(req: HttpRequest, state: web::Data<AppState>) -> Result<String, Error> {
  ...
  let result: Vec<models::Todo> = todos
    .limit(limit)
...
async fn update_todo(
  ...
  diesel::update(target)
    .set((title.eq(&info.title), description.eq(&info.description)))
    .execute(dbconn)
    .unwrap();
